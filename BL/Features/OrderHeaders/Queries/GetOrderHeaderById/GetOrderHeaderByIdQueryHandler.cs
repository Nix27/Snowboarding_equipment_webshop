...
namespace BL.Features.OrderHeaders.Queries.GetOrderHeaderById
{
    internal class GetOrderHeaderByIdQueryHandler : IRequestHandler<GetOrderHeaderByIdQuery, OrderHeaderDto?>
    {
        ...
        {
            var requestedOrderHeader = await _unitOfWork.OrderHeader.GetFirstOrDefaultAsync(o => o.Id == request.orderHeaderId, isTracked:request.isTracked, includeProperties:request.includeProperties);
